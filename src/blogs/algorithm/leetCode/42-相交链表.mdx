...
图示两个链表在节点 c1 开始相交：
![题目图片1](https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2018/12/14/160_statement.png)
题目数据 保证 整个链式结构中不存在环。  
...
最简单的方法，使用 Map 结构存储一条链表上的节点，然后遍历另一条，看在 Map 中是否能找到相同的节点。
如果找到即相交，找不到即没有相交。
