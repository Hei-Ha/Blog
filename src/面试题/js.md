...
为什么需要 bigInt? 用来表示 js 表示不了的数字，只支持整数，不支持小树。
js 能表示最大的安全数是正负 2^53 - 1

+ 为什么是 2^53？


    因为 js 中的所有数字，实际存储的都是一个 双精度的浮点数，并采用 IEEE 754 格式。
    最高的1位存储符号，其次11是指数，剩下的52位存储的是有效数字，所以 js 能安全表示的数字范围是 正负 2^53 - 1。
...
2、隐式绑定
这条规则是在函数调用的时候，是否有上下文对象，或者说是否被某个对象包含，
请看下面代码：
```javascript
...
1、创建（或者说构造）一个全新的对象
2、绑定 临时对象的原型 （赋值新对象的共有属性）
3、指定 this（新对象会被绑定到函数调用的 this）
4、执行构造方法 （赋值新对象的私有属性）
5、返回临时对象（如果函数没有返回其他对象，那么 new 表达式中的函数调用会自动返回这个刚才创造的新对象。）
